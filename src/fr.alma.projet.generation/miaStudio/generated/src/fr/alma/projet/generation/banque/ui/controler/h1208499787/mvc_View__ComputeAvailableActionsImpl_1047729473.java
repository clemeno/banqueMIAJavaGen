...
public class mvc_View__ComputeAvailableActionsImpl_1047729473 implements com.mia.studio.kernel.compiler.CompiledAction {

	public mvc_View__ComputeAvailableActionsImpl_1047729473() {
		//Do nothing
	}

	//For internal use only
	mvc_View__ComputeAvailableActionsImpl_1047729473(int anyInt, List anyList) {
			...
			try {			
				...
				throw new IllegalArgumentException(
						String.format(
								...
								"ComputeAvailableActionsImpl", argument.getClass().getName(),
	...
	public String runAction(java.lang.Object[] _action_parameters) {
		if (_action_parameters.length != 3)
			throw new IllegalArgumentException("Wrong number of parameters in call to ''ComputeAvailableActionsImpl''");
...
for (Action action : current.getActions()) {
	if(action.getName().equals("quitter")){
		res += manager.getString(action, "QuitActionImpl");
	}else{
		res += manager.getString(action, "AvailableActionImpl");
	}
}
